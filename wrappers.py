...
async def connectAsync(host=C.MANAGER_HOST, port=None, name=None,
                       ...
                       headless=False):
    @inlineCallbacks
    def worker(host, port, name, password, tls_mode, username, headless):
    ...
    p = await as_future(worker(host, port, name, password, tls_mode, username, headless))
...
class AsyncServerWrapper(object):
    ...
    async def _send(self, *args, **kw):
        @inlineCallbacks
        def __send(self, *args, **kw):
            """Send packet to this server."""
            value = yield self._cxn._send(self.ID, *args, **kw)
            returnValue(value)

        value = await as_future(__send(self, *args, **kw))
        return value
...
class ClientAsync(object):
    ...
    def _serverConnected(self, _c, data):
        ...
        ID, name = data
        try:
            asyncio.ensure_future(self._addServer(name, ID),
    ...
    async def refresh(self):
        ...
        async with self._refreshLock:
            actions = await as_future(_refresh(self))
